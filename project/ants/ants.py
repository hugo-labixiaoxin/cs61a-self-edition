        ...
        if not self.real_queen:
            ...
            while self.place.exit:
                if self.place.exit.ant:
                    if self.place.exit.ant not in self.ants:
                        self.place.exit.ant.damage*=2
                        self.ants.append(self.place.exit.ant)
                    if isinstance(self.place.exit.ant,ContainerAnt) and self.place.exit.ant.contained_ant is not None:
                        if self.place.exit.ant.contained_ant not in self.ants:
                            self.place.exit.ant.contained_ant.damage*=2
                            self.ants.append(self.place.exit.ant.contained_ant)
                else:
